...
- 2024-12-24：导航菜单重命名（“数据管理”->“数据资产”，“任务管理”->“决策推理”） [已完成]
  - 需求：修改顶部导航菜单项名称及对应页面标题。
  - 变更：
    1) 更新 `i18n/LanguageContext.tsx` 中的翻译文案。
    2) 更新 `Header.tsx` 及 `App.tsx` 中的状态匹配字符串。
    3) 修复页面内部硬编码的标题。
  - 文件：`src/i18n/LanguageContext.tsx`、`src/components/Header.tsx`、`src/App.tsx`

- 2025-12-24：开发空间列表增强（筛选、排序、新字段、列冻结）
  - 需求：增加状态筛选、镜像与规格排序；新增创建时间与创建者列；实现操作栏右侧固定冻结。
  - 变更：
    1) 在 `NotebookManagement.tsx` 中扩展 `NotebookInstance` 接口与 Mock 数据。
    2) 配置 `Table` 组件的 `filters`、`sorter` 和 `fixed` 属性。
    3) 调整表格 `scroll` 配置以支持横向滚动。
  - 文件：`src/components/NotebookManagement.tsx`
  - 验收：列表功能按预期增强，交互闭环且在大数据量/窄屏下体验良好。

- 2025-12-24：限制模板删除权限
  - 需求：系统内置模板（前三个）不允许删除，仅允许删除用户自己增加的“自定义”模板。
  - 变更：在 `HtmlConfigManagement.tsx` 中对“删除”按钮进行条件渲染，检查 `it.tag === '自定义'`。
  - 文件：`src/components/HtmlConfigManagement.tsx`
  - 验收：系统模板不再显示删除按钮，自定义模板显示删除按钮且功能正常。

- 2025-12-24：状态切换二次确认
  - 需求：点击任务模板的状态开关时增加二次确认弹窗，提醒用户会影响任务管理的创建任务。
  - 变更：修改 `Switch` 的 `onChange` 逻辑，使用 `Modal.confirm` 包裹状态更新。
  - 文件：`src/components/HtmlConfigManagement.tsx`
  - 验收：点击开关弹出确认框，确认后状态才发生变化并弹出提示。

- 2025-12-24：任务模板状态管理
  - 需求：任务模板列表增加“状态”列，支持开启/关闭开关，控制模板是否应用。
  - 变更：
    1) `TaskTemplate` 接口增加 `enabled` 属性。
    2) 列表渲染增加 `Switch` 组件及状态处理逻辑。
  - 文件：`src/components/HtmlConfigManagement.tsx`
  - 验收：点击开关可切换模板启用/禁用状态，并弹出操作反馈。

- 2025-12-24：任务模板管理功能优化
  - 需求：新增“新增任务模板”按钮及弹窗，支持输入名称和描述。创建后列表动态增加数据，点击“配置”进入详情编辑器。
  - 变更：
    1) 在 `HtmlConfigManagement.tsx` 中新增 `isAddModalOpen` 状态。
    2) 将模板列表改为动态 State 维护。
    3) 实现“新增任务模板”弹窗及列表更新逻辑。
    4) 优化“配置”按钮，使其携带对应模板数据进入编辑器。
  - 文件：`src/components/HtmlConfigManagement.tsx`
  - 验收：点击新增按钮弹出弹窗，提交后列表增加数据，点击配置可编辑该模板。

- 2025-12-24：登录持久化优化
  - 需求：修复新标签页打开时总是跳转登录页的问题，确保登录状态在会话间持久化。
  - 变更：统一 `src/App.tsx` 中 `localStorage` 的 Key 为 `limix_is_logged_in`。
  - 文件：`src/App.tsx`
  - 验收：登录后打开新标签页应直接进入系统，仅在点击“退出登录”后才返回登录页。

- 2025-12-24：Notebook 连接弹窗优化

  - 需求：去掉连接弹窗中“其他 IDE / 终端 接入”区块，简化连接选项。
  - 变更：移除 `src/App.tsx` 中的相关 UI 代码块。
  - 文件：`src/App.tsx`
  - 验收：连接弹窗内仅保留 VS Code 相关说明，不再显示“其他 IDE / 终端 接入”部分。

- 2025-12-18：角色管理-默认角色与UI优化
  - 需求：新增默认角色（不可删改），优化 UI（锁图标/徽章），并移除默认角色的编辑/设置/删除按钮。
  - 变更：
    1) 修改 `src/components/RoleManagement.tsx`，在 Mock 数据中新增 `isDefault` 角色；
    2) 添加 UI 逻辑：禁用 Switch，隐藏 Edit/Settings/Trash 按钮，仅保留 Assign User 按钮；
    3) 增加视觉标识：金色锁图标、“系统内置”徽章、绿色“长期生效”状态标签。
  - 文件：`src/components/RoleManagement.tsx`
  - 验收：默认角色显示且有特殊标识，操作受限；普通角色操作正常。

- 2025-12-18：批量操作优化与UI细节调整
  - 需求：将角色与用户的“批量删除”改为“批量启用/禁用”；优化部门管理UI（移除计数徽标、状态改为开关）。
  - 变更：
    1) `RoleManagement.tsx`：移除批量删除，新增批量启用/禁用；
    2) `DepartmentManagement.tsx`：新增用户批量启用/禁用；移除去重 Calendar/Tooltip 引用；移除 Header 处的 User Count Badge；状态列改为 Switch 组件。
  - 文件：`src/components/RoleManagement.tsx`, `src/components/DepartmentManagement.tsx`
  - 验收：批量操作功能正常转换为状态变更；部门用户列表状态可直接点击开关切换；界面无多余徽标。

- 2025-12-15：角色管理-移除操作日志按钮
  - 需求：去掉角色管理页面内的操作日志按钮。
  - 变更：移除 `src/components/RoleManagement.tsx` 中的操作日志按钮代码。
  - 文件：`src/components/RoleManagement.tsx`
  - 验收：角色管理页面不再显示操作日志按钮。

- 2025-12-17：个人中心重构（垂直布局+弹窗改密）
  - 需求：取消标签页布局，合并基本信息/通知/日志为垂直流式页面；新增密码修改按钮与弹窗。
  - 变更：重构 `src/components/PersonalCenter.tsx`，移除 Tabs，新增 Dialog。
  - 文件：`src/components/PersonalCenter.tsx`
  - 验收：页面为垂直布局，点击相关按钮可弹出密码修改窗口。

- 2025-12-15：系统管理-隐藏个人中心
  - 需求：隐藏系统管理内的个人中心入口。
  - 变更：移除 `src/components/SystemManagement.tsx` 中的个人中心菜单项与快速操作入口。
  - 文件：`src/components/SystemManagement.tsx`
  - 验收：系统管理侧边栏与概览页不再显示个人中心相关入口。
