...
import { Calendar as UiCalendar } from "./ui/calendar"; // Renamed to avoid conflict with lucide-react Calendar
// Actually package.json has dayjs, let's use dayjs for logic but Calendar might expect Date objects.
...
const TaskManagement: React.FC<TaskManagementProps> = ({
  ...
  const forecastingMockData = useMemo(() => {
    ...
    for (let i = 0; i < 365; i++) {
      data.push({
        ...
        timestamp: baseDate.add(i, 'day').valueOf(),
        // 模拟趋势 + 周期 + 随机噪声
        value: 50 + Math.sin(i / 30) * 10 + Math.random() * 5 + (i / 10)
      });
    }
    return data;
  }, []);
  ...
  const forecastingSplitState = useMemo(() => {
    if (formData.taskType !== TASK_TYPES.forecasting) return { splitIndex: 0, splitPercentage: 0 };

    let splitIndex = 0;
    let splitPercentage = 0;

    if (formData.forecastingConfig.splitMode === 'percent') {
      splitPercentage = formData.forecastingConfig.trainRatio;
      splitIndex = Math.floor(forecastingMockData.length * (splitPercentage / 100));
    } else {
      // 日期划分模式
      const startDate = dayjs('2025-01-01');
      const endDate = dayjs('2025-12-31');
      const splitDate = formData.forecastingConfig.splitDate ? dayjs(formData.forecastingConfig.splitDate) : null;

      if (splitDate) {
        // 计算日期在模拟范围内的位置
        const totalDays = endDate.diff(startDate, 'day');
        const diffDays = splitDate.diff(startDate, 'day');

        // 归一化到 0-100%
        let ratio = (diffDays / totalDays) * 100;
        // 限制范围
        if (ratio < 0) ratio = 0;
        if (ratio > 100) ratio = 100;

        splitPercentage = ratio;
        splitIndex = Math.floor(forecastingMockData.length * (ratio / 100));
      } else {
        // 未选择日期时默认显示在中间或0
        splitPercentage = 0;
        splitIndex = 0;
  ...
  }, [formData.taskType, formData.forecastingConfig.splitMode, formData.forecastingConfig.trainRatio, formData.forecastingConfig.splitDate, forecastingMockData]);
  ...
  const validateForm = (): boolean => {
    ...
    if (formData.taskType === TASK_TYPES.forecasting) {
      ...
      if (!cc) {
        ...
        if (formData.selectedFiles.length === 2) {
          if (!cc.trainFile || !cc.testFile) {
            errors.classificationSplit = '请分别选择训练集文件和测试集文件';
          } else if (cc.trainFile === cc.testFile) {
            errors.classificationSplit = '训练集和测试集不能是同一个文件';
          }
        } else {
          // 常规比例验证
      ...
      if (!rc) {
        ...
        // 回归任务：根据文件数量选择验证逻辑
        if (formData.selectedFiles.length === 2) {
          if (!rc.trainFile || !rc.testFile) {
            errors.regressionSplit = '请分别选择训练集文件和测试集文件';
          } else if (rc.trainFile === rc.testFile) {
            errors.regressionSplit = '训练集和测试集不能是同一个文件';
          }
        } else {
  ...
  /**
   * 自动检测训练集和测试集文件
   * 当选中文件正好为2个时，尝试根据文件名关键词自动预填
   */
  useEffect(() => {
    if (formData.selectedFiles.length !== 2) return;

    const [file1, file2] = formData.selectedFiles;
    const lower1 = file1.toLowerCase();
    const lower2 = file2.toLowerCase();

    // 关键词检测
    const isTrain1 = /train|训练/.test(lower1);
    const isTest1 = /test|测试/.test(lower1);
    const isTrain2 = /train|训练/.test(lower2);
    const isTest2 = /test|测试/.test(lower2);

    let trainFile = '';
    let testFile = '';

    // 逻辑：如果一个包含train且另一个不包含train（或者包含test），则认定前者为训练集
    if (isTrain1 && !isTrain2) {
      trainFile = file1;
      testFile = file2;
    } else if (isTrain2 && !isTrain1) {
      trainFile = file2;
      testFile = file1;
    } else if (isTest1 && !isTest2) {
      testFile = file1;
      trainFile = file2; // 另一文件默认为训练集
    } else if (isTest2 && !isTest1) {
      testFile = file2;
      trainFile = file1;
    }
    ...
    if (trainFile && testFile) {
      if (formData.taskType === TASK_TYPES.classification) {
        // 仅当当前未设置或设置不完整时才覆盖，避免用户手动修改后被重置
        // 但这里为了响应文件变化，如果原来的选择不在新文件列表中，也应该更新
        // 简单策略：如果当前trainFile/testFile均为空，或者不合法，则自动填充
  ...
  }, [formData.selectedFiles, formData.taskType]);
        ...
        return {
          ...
          forecastingConfig: (typeof parsedCfg === 'object' && parsedCfg?.forecasting)
            ? {
              ...
              splitMode: parsedCfg?.forecasting.splitMode || 'percent',
              trainRatio: parsedCfg?.forecasting.trainRatio || 80,
              testRatio: parsedCfg?.forecasting.testRatio || 20,
              splitDate: parsedCfg?.forecasting.splitDate ? new Date(parsedCfg.forecasting.splitDate) : undefined
            }
            : {
              timeColumn: '',
              targetColumn: '',
                        ...
                        <div className="space-y-2">
                          {availableModels
                            ...
                            .map(model => (
                              <div
                                key={model.id}
                                className={`border rounded-lg p-3 cursor-pointer transition-colors ${formData.models.includes(model.id)
                        ...
                        {formData.taskType === TASK_TYPES.forecasting && (
                          <div className="space-y-4">
                            ...
                            <div className="space-y-4 pt-4 border-t">
                              <h4 className="text-sm font-medium">数据划分配置</h4>

                              <RadioGroup
                                value={formData.forecastingConfig.splitMode}
                                onValueChange={(value: 'percent' | 'date') => handleInputChange('forecastingConfig', {
                                  ...formData.forecastingConfig,
                                  splitMode: value
                                })}
                              ...
                              >
                                <div className="flex items-center space-x-2">
                                  ...
                                  <Label htmlFor="mode-percent">百分比划分</Label>
                                </div>
                                <div className="flex items-center space-x-2">
                                  <RadioGroupItem value="date" id="mode-date" />
                                  <Label htmlFor="mode-date">特定日期划分</Label>
                                </div>
                              </RadioGroup>
                              ...
                              {formData.forecastingConfig.splitMode === 'percent' ? (
                                <div className="grid grid-cols-2 gap-4">
                                  <div>
                                    <Label htmlFor="fc-train-ratio">训练集比例(%)</Label>
                                    <Input
                                      id="fc-train-ratio"
                                      type="number"
                                  ...
                                  <div>
                                    <Label htmlFor="fc-test-ratio">测试集比例(%)</Label>
                                    <Input
                                      id="fc-test-ratio"
                                      type="number"
                                ...
                                <div>
                                  ...
                                  <Popover>
                                    <PopoverTrigger asChild>
                                      <Button
                                        variant={"outline"}
                                        className={`w-full justify-start text-left font-normal ${!formData.forecastingConfig.splitDate && "text-muted-foreground"}`}
                                      >
                                    ...
                                    <PopoverContent className="w-auto p-0" align="start">
                                      <UiCalendar
                                        ...
                                        onSelect={(date: Date | undefined) => handleInputChange('forecastingConfig', {
                                          ...formData.forecastingConfig,
                                          splitDate: date
                                        })}
                                        initialFocus
                              ...
                              {/* 历史数据划分可视化 */}
                              <div
                                className="mt-4 p-4 rounded-lg border border-slate-700"
                                style={{ backgroundColor: '#0f172a' }} // 强制深色背景 (slate-900) 以确保图表可见
                              >
                                <Label className="mb-2 block text-slate-300">历史数据划分可视化预览 (基于模拟数据)</Label>
                                    ...
                                    >
                                      <defs>
                                        <linearGradient id="splitColor" x1="0" y1="0" x2="1" y2="0">
                                          <stop
                                            offset={`${forecastingSplitState.splitPercentage}%`}
                                            stopColor="#60a5fa"
                                            stopOpacity={0.5}
                                          />
                                          <stop
                                      ...
                                      />
                                      <ReferenceLine
                                        x={forecastingMockData[Math.min(forecastingSplitState.splitIndex, forecastingMockData.length - 1)]?.date}
                                        stroke="#f59e0b"
                                        strokeDasharray="3 3"
                                        label={{
                                          value: formData.forecastingConfig.splitMode === 'date' && formData.forecastingConfig.splitDate
                                            ? dayjs(formData.forecastingConfig.splitDate).format('YYYY-MM-DD')
                                            : `${forecastingSplitState.splitPercentage.toFixed(0)}%`,
                                          fill: '#f59e0b',
                                          fontSize: 12,
                                          position: 'top'
                                        }}
                                ...
                                <div className="flex justify-between text-xs text-slate-400 mt-2 px-2">
                                  <div className="flex items-center gap-2">
                                    ...
                                    <span>训练集 (Training)</span>
                                  </div>
                                  <div className="flex items-center gap-2">
                                    <div className="w-3 h-3 bg-amber-400/50 rounded-sm"></div>
                                    <span>测试集 (Test)</span>
                                  </div>
                                </div>
                              </div>
                        ...
                        {formData.taskType === TASK_TYPES.classification && (
                          <div className="space-y-4">
                            {formData.selectedFiles.length === 2 ? (
                              ...
                              <div className="grid grid-cols-2 gap-4">
                                <div>
                                  <Label htmlFor="class-train-file">训练集文件 (Training Set)</Label>
                                  <Select
                                    value={formData.classificationConfig.trainFile || ''}
                                    onValueChange={(value: string) => {
                                      handleInputChange('classificationConfig', {
                                        ...formData.classificationConfig,
                                        trainFile: value,
                                        // 如果选择的文件等于当前测试集文件，则清空测试集选择，避免冲突
                                        testFile: formData.classificationConfig.testFile === value ? '' : formData.classificationConfig.testFile
                                      });
                                    }}
                                  >
                                    <SelectTrigger>
                                      <SelectValue placeholder="选择训练集文件" />
                                    </SelectTrigger>
                                    <SelectContent>
                                ...
                                <div>
                                  <Label htmlFor="class-test-file">测试集文件 (Test Set)</Label>
                                  <Select
                                    value={formData.classificationConfig.testFile || ''}
                                    onValueChange={(value: string) => {
                                      handleInputChange('classificationConfig', {
                                        ...formData.classificationConfig,
                                        testFile: value,
                                        trainFile: formData.classificationConfig.trainFile === value ? '' : formData.classificationConfig.trainFile
                                      });
                                    }}
                                  >
                                    <SelectTrigger>
                                      <SelectValue placeholder="选择测试集文件" />
                                    </SelectTrigger>
                                    <SelectContent>
                                      {formData.selectedFiles.map(f => (
                                        <SelectItem key={f} value={f}>{f}</SelectItem>
                        ...
                        {formData.taskType === TASK_TYPES.regression && (
                          <div className="space-y-4">
                            {formData.selectedFiles.length === 2 ? (
                              ...
                              <div className="grid grid-cols-2 gap-4">
                                <div>
                                  <Label htmlFor="reg-train-file">训练集文件 (Training Set)</Label>
                                  <Select
                                    value={formData.regressionConfig.trainFile || ''}
                                    onValueChange={(value: string) => {
                                      handleInputChange('regressionConfig', {
                                        ...formData.regressionConfig,
                                        trainFile: value,
                                        testFile: formData.regressionConfig.testFile === value ? '' : formData.regressionConfig.testFile
                                      });
                                    }}
                                  >
                                    <SelectTrigger>
                                ...
                                <div>
                                  <Label htmlFor="reg-test-file">测试集文件 (Test Set)</Label>
                                  <Select
                                    value={formData.regressionConfig.testFile || ''}
                                    onValueChange={(value: string) => {
                                      handleInputChange('regressionConfig', {
                                        ...formData.regressionConfig,
                                        testFile: value,
                                        trainFile: formData.regressionConfig.trainFile === value ? '' : formData.regressionConfig.trainFile
                                      });
                                    }}
                                  >
                                    <SelectTrigger>
